...
最新: [詳細キャッシュのテストと分割](docs/cache-results.md)。テストは67件。保存処理を無変換で分割しました。次はページ遷移監視（推奨Astra中）です。

v14.1の主要定義を段階的に別ファイルへ分けています。22個の断片を元の順で連結し、生成物が原本とバイト単位で完全一致することを強制します。baselineは比較専用、srcがビルド入力です。各区分内部の責務分離は未完了です。
