...
最新: [表示状態の分割結果と残作業](docs/view-state-results.md)。テストは43件。表示状態モデルを別ファイルに移しました。動作は無変更です。

v14.1のイベント・保存処理・設定管理・論理ルール・動画情報取得・情報反映・表示状態を別ファイルへ分けました。11個の断片を元の順で連結し、生成物が原本とバイト単位で完全一致することを強制します。baselineは比較専用、srcがビルド入力です。
