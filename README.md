# Nico Nico Ranking NG — Task 005A

最新: [動画モデル反映の準備](docs/detail-model-preparation.md)。テストは37件。情報反映・共有・通知順を確認しました。本体は無変更です。[Task 005A結果](docs/task-005a-results.md)も参照できます。

v14.1のイベント・保存処理・設定管理・論理ルール・動画情報取得を別ファイルへ分けました。8つの断片を元の順で連結し、生成物が原本とバイト単位で完全一致することを強制します。baselineは比較専用、srcがビルド入力です。
