最新160.14: [残存する投稿者名の補完とキャッシュ改善](docs/owner-names-16014.md)。同じ動画・投稿者だと確認できた名前を表示と名前NGへ反映。初期カードと名前だけの取得結果もキャッシュし、復元を通信開始より前に実施。GitHub反映と実サイトでの新版測定は保留。
...
生成物は `dist/nico-nico-ranking-ng-v16-list-tile-fix.user.js`（メタデータ版番号160.14）です。baselineのスクリプト原本は編集しないでください。原本のSHA-256検証は引き続き必須です。ビルドは分割ソースの順序どおりの連結と出力一致を検証します。
