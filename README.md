...
最新: [動画情報の接続部分の分割結果](docs/detail-listener-results.md)。テストは37件。情報反映の接続部分を分割しました。動作は無変更です。

v14.1のイベント・保存処理・設定管理・論理ルール・動画情報取得・情報反映の接続部分を別ファイルへ分けました。10個の断片を元の順で連結し、生成物が原本とバイト単位で完全一致することを強制します。baselineは比較専用、srcがビルド入力です。
