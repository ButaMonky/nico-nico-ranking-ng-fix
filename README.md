...
最新: [診断と新規タブ処理の分割結果](docs/services-results.md)。テストは49件。テスト準備から分割・検証まで完了しました。動作は無変更です。

v14.1のイベント・保存処理・設定管理・論理ルール・動画情報取得・情報反映・表示状態・診断・新規タブ処理を別ファイルへ分けました。14個の断片を元の順で連結し、生成物が原本とバイト単位で完全一致することを強制します。baselineは比較専用、srcがビルド入力です。
