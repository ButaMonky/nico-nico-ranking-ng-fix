最新160.13: [匿名診断と実通信の集計](docs/diagnostics-16013.md)。共有用のコピー機能、実通信・省略・キャッシュの区別、SPAごとの集計を追加。GitHub反映と実サイトでの新版測定は保留。
...
生成物は `dist/nico-nico-ranking-ng-v16-list-tile-fix.user.js`（メタデータ版番号160.13）です。baselineのスクリプト原本は編集しないでください。原本のSHA-256検証は引き続き必須です。ビルドは分割ソースの順序どおりの連結と出力一致を検証します。
