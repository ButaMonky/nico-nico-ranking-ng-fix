最新160.16: [通常表示の投稿者欄と初期待機の改善](docs/compact-owner-16016.md)。▼未展開時にも補完名を表示。カード以外の装飾要素で初期処理が15秒待つケースを修正。次のUI・論理回路NG・プレビューの作業単位も整理。GitHub反映は保留。
...
生成物は `dist/nico-nico-ranking-ng-v16-list-tile-fix.user.js`（メタデータ版番号160.16）です。baselineのスクリプト原本は編集しないでください。原本のSHA-256検証は引き続き必須です。ビルドは分割ソースの順序どおりの連結と出力一致を検証します。
