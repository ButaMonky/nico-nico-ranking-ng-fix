# Nico Nico Ranking NG — Task 004A

最新: [Task 005A準備](docs/task-005a-preparation.md)。テストは33件。動画情報取得の制御を模擬通信で確認しました。取得処理は無変更です。[Task 004A結果](docs/task-004a-results.md)も参照できます。

v14.1のイベント・保存処理・設定管理・論理ルールを別ファイルへ分けました。7つの断片を元の順で連結し、生成物が原本とバイト単位で完全一致することを強制します。baselineは比較専用、srcがビルド入力です。
