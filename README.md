最新160.15: [非公開表示の誤認と投稿者欄の更新漏れ修正](docs/hidden-owner-16015.md)。DOMの「投稿者非公開」を実名と誤認して名前補完を省く問題、自動追加カードの通常表示が更新されない問題を修正。GitHub反映は保留。
...
生成物は `dist/nico-nico-ranking-ng-v16-list-tile-fix.user.js`（メタデータ版番号160.15）です。baselineのスクリプト原本は編集しないでください。原本のSHA-256検証は引き続き必須です。ビルドは分割ソースの順序どおりの連結と出力一致を検証します。
