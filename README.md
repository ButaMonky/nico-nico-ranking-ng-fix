...
最新: [操作・起動境界の分割結果と全体進捗](docs/controller-start-results.md)。テストは61件。動作は無変更です。Main内部の整理と実環境検証は残っています。

v14.1の主要定義を段階的に別ファイルへ分けています。20個の断片を元の順で連結し、生成物が原本とバイト単位で完全一致することを強制します。baselineは比較専用、srcがビルド入力です。各区分内部の責務分離は未完了です。
