...
最新: [ページ処理の分割結果](docs/pages-results.md)。テストは59件。ページ処理を既存の境界で分けました。動作は無変更です。

v14.1の主要定義を段階的に別ファイルへ分けています。18個の断片を元の順で連結し、生成物が原本とバイト単位で完全一致することを強制します。baselineは比較専用、srcがビルド入力です。各区分内部の責務分離は未完了です。
