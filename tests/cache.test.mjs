...
async function harness(path,raw=null,failWrite=false){
 ...
 const context={window:{},Date:{now:()=>now},sessionStorage:storage,console:{warn:(...a)=>warnings.push(a)}};
 ...
 return {factory,warnings,writes,raw:()=>raw,now:()=>now,advance:ms=>now+=ms};
}
for(const [label,path] of [['baseline',baseline],['generated',output]]){
 test(`${label}: cache persistence, singleton reconfiguration and reload`,async()=>{
  ...
  assert.equal(c.get('42').title,'test');assert.equal(JSON.parse(h.raw()).schema,2);
 ...
 test(`${label}: TTL boundary and oldest-entry eviction`,async()=>{
  ...
  assert.equal(c.diagnostics().stats.expired,1);assert.deepEqual(JSON.parse(h.raw()).entries,[]);
 ...
 test(`${label}: corrupt data, unsupported schema and failed writes remain usable`,async()=>{
  ...
  assert.equal(c.has('a'),true);assert.equal(c.diagnostics().stats.saves,0);assert.equal(h.warnings.length,1);
