...
async function load(path,extra={}){
 ...
 {NRN_VERSION:'160.18',window,location:{href:'https://www.nicovideo.jp/tag/test',origin:'https://www.nicovideo.jp'},URL,
...
for(const [label,path] of [['baseline',baseline],['generated',output]]){
 test(`${label}: diagnostics retention and snapshots`,async()=>{
  ...
  assert.equal(d.snapshot().recent.length,30);assert.equal(d.snapshot().version,label==='baseline'?'14.1':'160.18');
